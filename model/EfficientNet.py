...
class EfficientNet(nn.Module):
    def __init__(self, CompoundScale: list, dropout_ratio=0.2,num_classes=100 ): # CompoundScale ->[width_mul, depth_mul]
        ...
        # BiFPN
        self.contain=[]
        for idx in range(len(self.B0_architecture)):
            self.contain.append(self.B0_architecture[idx][1])
        ...
        for e, c, n, s, k in self.B0_architecture:
...
if __name__=='__main__':
    ...
    CompoundScale=params[modelName][:2]
    model=EfficientNet(CompoundScale)
    ch_out=[]
    for idx in range(len(model.B0_architecture)):

        ch_out.append(model.B0_architecture[idx][1])
